...
výhodou je silná korekcia chýba, ktorá umožňuje prečítať identifikátor aj pri
...
Systém umožní obsluhe výrobnej linky ovládať PLC zariadenia prostredníctvom
mobilnej aplikácie. Identifikácia zariadenia prebieha skenovaním ArUco
identifikátora umiestneného priamo na zariadení. Po naskenovaní sa aplikácia
pripojí k~zariadeniu a~obsluha bude môcť odosielať príkazy a~sledovať aktuálny
stav.
...
funkciám má prístup. Keďže ku jednému zariadeniu sa môže pokúsiť pripojiť
...
ktorý používateľ príkaz vydal a~na ktoré zariadenie. Administrátor a manažér
...
znalostnej bázy bude možné viazať na konkrétne zariadenie.
...
\begin{description}
      ...
      \item[Manažér:] Zameriava sa na sledovanie udalostí. Má prístup k~histórii prihlásení
            ...
            Takisto má prístup k~AI asistentovi a môže upravovať vedomostnú bázu. Linku
            priamo neovláda no vidí kto je prihlásený a aké operácie vykonáva.
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      \toprule
      \textbf{ID}    & UC-02: Skenovanie ArUco identifikátora a spárovanie linky                   \\
      \textbf{Aktér} & Operátor, údržbár, admin                                                    \\
      \textbf{Popis} & Používateľ naskenuje ArUco identifikátor pri zariadení.
...
a~nevyžadujú dodatočnú vrstvu. FastAPI bolo zvolené aj z~ďalších
praktických dôvodov. Asynchrónna architektúra (\acrshort{asgi}) umožňuje
súbežne obsluhovať požiadavky mobilnej aplikácie aj dlhotrvajúce volania
jazykového modelu bez vzájomného blokovania. Automaticky generovaná OpenAPI
dokumentácia uľahčuje ladenie \acrshort{api} počas vývoja. Validácia vstupov
cez Pydantic redukuje množstvo pomocného kódu. Rolu zohrali aj predchádzajúce
skúsenosti s~týmto frameworkom v bakalárskej práci, čo výrazne urýchlilo vývoj
a umožnilo sa sústrediť na implementáciu biznis logiky a~integráciu AI
asistenta.
...
správu relácií a~riadenie zariadení. Samostatná služba obsluhuje AI asistenta
...
systémy výrobných liniek. Edge zariadenie plní úlohu centrálneho uzla, beží na
ňom hlavný backend aj AI asistent a všetka logika spracovania príkazov sa
...
%%% tu mozno tabulka s rolami a co mozu roly robit
...
Prihlásenie na zariadenie prebieha volaním \texttt{POST
      /devices/\{aruco\_id\}/login}. Backend overí, či je zariadenie voľné. Ak áno,
...
SYSTEM_PROMPT_SK = """Tvoje meno je PLEXO. Si priemyselný asistent aplikácie Scan2Control na riadenie PLC zariadení cez ArUco identifikátory.
{intro_instruction}
PRAVIDLÁ:
