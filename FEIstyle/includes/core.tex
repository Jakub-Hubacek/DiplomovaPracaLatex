...
k~výrobným linkám, identifikácia fyzických zariadení, nasadenie na edge, otvorenosť
...
výrobnej linky. Edge zariadenia dokážu filtrovať a vyhodnocovať dáta lokálne, do
...
funkciám má prístup. Keďže ku jednej výrobnej linke sa môže pokúsiť pripojiť
...
\begin{description}
      \item[Operátor:] Základná obsluha výrobnej linky. Prihlasuje sa na konkrétnu linku
            ...
            stav linky v~reálnom čase. Nemá prístup k~reportom, správe vedomostnej
      ...
      \item[Manažér:] Zameriava sa na sledovanie udalostí. Má prístup k~histórii prihlásení
            ...
            Výrobnú linku priamo neovláda, no vidí kto je prihlásený a aké operácie vykonáva.
      ...
      \item[Údržbár:] Má exkluzívnu ochranu relácie a žiadny iný používateľ ho nemôže
            ...
            Spravuje vedomostnú bázu pre jednotlivé linky, môže upravovať existujúce linky a vytvárať nové.
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      \toprule
      \textbf{ID}    & UC-02: Skenovanie ArUco identifikátora a spárovanie výrobnej linky              \\
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      ...
      Vykonanie príkazu je potvrdené a je zobrazený čas od odoslania po prijatie príkazu linkou. \\
      \bottomrule
\end{tabularx}
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      ...
      Odpovedá výhradne na základe vedomostnej bázy, či už všeobecnej alebo viazanej ku konkrétnej linke. Ak informácia chýba, prizná to. Rozhranie aj odpovede sú lokalizované v~SK a~EN. \\
      \bottomrule
\end{tabularx}
...
všetkých liniek naraz. Node-RED subscribuje selektívne len na témy
\texttt{commands} pre linky, ktoré spravuje. Príkazy sú publikované s QoS
...
samostatná služba vo vlastnom kontajneri, preto má aj vlastnú databázovú
...
WebSocket pre aktualizácie stavu linky.
...
\begin{table}[H]
      ...
      \resizebox{\textwidth}{!}{%
            \begin{tabular}{|l|l|l|l|}
                  ...
                  PUT             & /plc/devices/\{aruco\_id\}                 & Úprava linky                 & admin, údržbár           \\
                  DELETE          & /plc/devices/\{aruco\_id\}                 & Vymazanie linky              & admin, údržbár           \\
                  ...
                  POST            & /devices/\{aruco\_id\}/login               & Prihlásenie na linku         & admin, údržbár, operátor \\
                  ...
                  GET             & /command-logs/device/\{aruco\_id\}         & Logy linky                   & admin, manažér, údržbár  \\
                  \hline
...
Prihlásenie na linku prebieha volaním \texttt{POST
      /devices/\{aruco\_id\}/login}. Backend overí, či je linka voľná. Ak áno,
...
presnosť odpovedí pri otázkach viazaných na konkrétny stroj.
...
hodnoty premennej z linky. Keď táto zmena dorazí, zobrazí sa snackbar
...
identifikuje. Po úspešnom naskenovaní je používateľ priradený k danej
linke a získava prístup k jeho monitorovaniu a ovládaniu. Tento prístup
...
identifikátora je vyvýšená nad čiernou základňou. Fyzická značka sa následne
umiestni na linku, čím sa prepojí záznam v systéme s reálnou linkou v
prevádzke.

\subsection{Monitoring a ovládanie liniek}
\textbf{Dostupné pre:} operátor, údržbár, administrátor

Monitoring a ovládanie liniek predstavuje hlavnú funkcionalitu aplikácie. Po
prihlásení na linku má používateľ k dispozícii prehľad aktuálneho stavu
linky. Môže sledovať stavy senzorov, aktuálny režim
prevádzky, stav v ktorom sa linka nachádza, a iné prevádzkové parametre.
...
\subsection{Správa liniek}
\textbf{Dostupné pre:} údržbár, manažér, administrátor

Správa liniek umožňuje udržiavať databázu liniek aktuálnu a v súlade so
...
reálny stav liniek v prevádzke.
...
svoje prístupové heslo. Tieto nastavenia sú viazané na konkrétny používateľský
účet a zostávajú zachované aj po opätovnom prihlásení.
...
\hline
\textbf{Vzdialenosť} & \textbf{0$^{\circ}$} & \textbf{15$^{\circ}$} & \textbf{30$^{\circ}$} & \textbf{45$^{\circ}$} \\
\hline
...
Asistent bol nasadený s modelom Claude Haiku~4.5 cez Anthropic API a vektorovou databázou ChromaDB, do ktorej bola vložená znalostná báza pripravená v rámci tejto práce. Pre vyhodnotenie bola použitá rovnaká metodika ako v kapitole \ref{sec:assistant}, no aplikovaná na produkčné nasadenie cez Anthropic API namiesto lokálnych modelov. Sada obsahovala 15 testovacích otázok rozdelených do troch kategórií. Prvá kategória obsahuje otázky priamo pokryté v znalostnej báze, druhá otázky vyžadujúce kombináciu informácií z viacerých záznamov a tretia otázky, ktoré v báze nie sú a očakáva sa použitie nástroja na vyhľadávanie na internete.

Každá odpoveď bola hodnotená v troch kritériách na škále 1 až 5: vecná správnosť, úplnosť a zrozumiteľnosť. Hodnotenie vykonal autor práce rovnako ako v kapitole \ref{sec:assistant}. Výsledky sú v tabuľke \ref{tab:test-asistent}.
...
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Kategória otázok} & \textbf{Počet} & \textbf{Správnosť} & \textbf{Úplnosť} & \textbf{Zrozumiteľnosť} \\
\hline
