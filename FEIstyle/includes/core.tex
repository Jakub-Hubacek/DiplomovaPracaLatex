...
k~výrobným zariadeniam, identifikácia fyzických zariadení, nasadenie na edge, otvorenosť
...
\begin{itemize}
      ...
      \item \textbf{Identifikácia zariadení:} Platforma natívne nepodporuje
            skenovanie unikátnych identifikátorov. Navigácia cez \acrshort{qr} identifikátory je
...
lokálnom zariadení alebo v~blízkosti monitorovanej alebo ovládanej
...
výrobného zariadenia. Komunikácia výrobného zariadenia a aplikácie prebieha cez edge zariadenie umiestnené priamo v~prevádzke.

\subsection{Opis systému}

Systém umožní obsluhe ovládať PLC výrobného zariadenia prostredníctvom mobilnej aplikácie.
Identifikácia výrobného zariadenia prebieha skenovaním ArUco identifikátora umiestneného
priamo na zariadení. Po naskenovaní sa aplikácia pripojí k~výrobnému zariadeniu a~obsluha
...
funkciám má prístup. Keďže ku jednému výrobnému zariadeniu sa môže pokúsiť pripojiť
...
ktorý používateľ príkaz vydal, a~na ktoré výrobné zariadenie. Administrátor a manažér
budú môcť spravovať používateľské účty a~schvaľovať nové registrácie.

Súčasťou systému bude inteligentný asistent, ktorý bude odpovedať na technické
otázky obsluhy na základe dokumentácie nahratej v~znalostnej báze. Obsah
znalostnej bázy bude možné viazať na konkrétne výrobné zariadenie.
...
\begin{description}
      \item[Operátor:] Základná obsluha výrobného zariadenia. Prihlasuje sa na konkrétne zariadenie
            skenovaním ArUco identifikátora, odosiela riadiace príkazy a~sleduje aktuálny
            stav zariadenia v~reálnom čase. Nemá prístup k~reportom, správe vedomostnej
      ...
      \item[Manažér:] Zameriava sa na sledovanie udalostí. Má prístup k~histórii prihlásení
            ...
            Takisto má prístup k~AI asistentovi a môže upravovať vedomostnú bázu.
            Výrobné zariadenie priamo neovláda, no vidí kto je prihlásený a aké operácie vykonáva.
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      \toprule
      \textbf{ID}    & UC-02: Skenovanie ArUco identifikátora a spárovanie výrobného zariadenia              \\
      \textbf{Aktér} & Operátor, údržbár, admin                                                    \\
      \textbf{Popis} & Používateľ naskenuje ArUco identifikátor pri výrobnom zariadení.
      Ak je zariadenie voľné, po potvrdení dialógu sa vytvorí relácia.
      Ak je obsadené, zobrazí sa identita a~rola prihláseného.\\
      \bottomrule
\end{tabularx}
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      \toprule
      \textbf{ID}    & UC-03: Ovládanie výrobného zariadenia                                                   \\
      \textbf{Aktér} & Operátor, údržbár, admin                                                      \\
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      \toprule
      \textbf{ID}    & UC-04: Monitorovanie stavu výrobného zariadenia                                                                                   \\
      \textbf{Aktér} & Operátor, údržbár, admin                                                                                                \\
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      ...
      \textbf{Popis} & Je možné prezeranie logov a reportov s filtrovaním podľa časového obdobia a zariadenia. \\
      \bottomrule
\end{tabularx}
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      ...
      \textbf{Popis} & Manažér a admin schvaľujú registrácie, priraďujú roly, upravujú a~odstraňujú kontá. Hlavného admina nie je možné odstrániť ani upraviť inou cestou. \\
      \bottomrule
\end{tabularx}
...
\begin{description}
      \item [FP1:] Aplikácia musí umožniť načítanie unikátneho identifikátora na automatické spárovanie mobilného zariadenia s konkrétnou výrobným zariadením.
      ...
      \item [FP5:] Implementácia chatbota využívajúceho metódu \textit{Retrieval-Augmented Generation}. Chatbot musí odpovedať na technické otázky výhradne na základe poskytnutej vedomostnej bázy. Ak čerpá z iného zdroja, musí to jasne uviesť.
      \item [FP6:] Plná lokalizácia používateľského rozhrania a odpovedí chatbota v dvoch jazykoch: slovenský (SK) a anglický (EN).
      \item [FP7:] Aplikácia musí implementovať mechanizmus exkluzívnej relácie pre ovládanie konkrétneho zariadenia. Po úspešnom prihlásení sa vytvorí aktívna relácia viazaná na používateľa. Relácia ostáva aktívna až do jej explicitného ukončenia odhlásením používateľa alebo prevzatím iným používateľom za podmienok definovaných v~FP8.
      \item [FP8:] Pri pokuse o spárovanie zariadenia, ktoré je už obsadené aktívnou reláciou, musí aplikácia používateľovi zobraziť informáciu o existujúcej relácii (používateľa/rola). Nútené ukončenie resp. prevzatie relácie musí byť umožnené v prípade neodhlásenia sa už neprítomnej obsluhy.
      ...
      \item [FP10:] Systém musí ukladať základný log operácií obsahujúci minimálne: čas vykonania, identifikátor používateľa, identifikátor zariadenia, typ akcie/príkazu a výsledok vykonania. Log musí byť dostupný pre roly \textit{manažér}, \textit{údržbár} a \textit{admin}.
\end{description}
...
\begin{description}
      ...
      \item [DP3:] Systém si musí zachovať kľúčovú funkcionalitu, teda ovládanie výrobného zariadenia aj v prípade výpadku pripojenia k internetu.
\end{description}
...
Riadiaci systém výrobného zariadenia je PLC od firmy Siemens, konkrétne model
...
frameworkom v bakalárskej práci, čo výrazne urýchlilo vývoj a umožnilo sa
sústrediť na implementáciu biznis logiky a~integráciu AI asistenta.
...
Pre jednoznačnú identifikáciu výrobného zariadenia bolo v~rámci práce spočiatku
...
bolo možné použitie frameworku ako Vuforia, čo by ale značne zvýšilo závislosť
...
\acrshort{api} od spoločnosti Anthropic, nakoľko v poslednej dobe zaznamenal
...
plnú funkčnosť aj bez pripojenia k internetu. Mobilná aplikácia komunikuje
výhradne v rámci lokálnej siete závodu a v súčasnosti sa pripojenie na internet
využíva len pre účely AI asistenta. Ak by sme využívali lokálne spustiteľný
...
správu relácií a~riadenie výrobného zariadenia. Samostatná služba obsluhuje AI asistenta
...
výrobného zariadenia. Všetky tieto komponenty okrem mobilnej aplikácie a
...
aktualizácie stavu zariadenia cez WebSocket. Príkazy odoslané z aplikácie putujú
...
ďalšie výrobné zariadenia bez zmeny základnej topológie. V súčasnej implementácii je
na switch napojený jeden riadiaci systém Siemens S7-1200.

\subsection{Štruktúra MQTT tém}

Komunikácia medzi backend službou, Node-RED a výrobnými zariadeniami prebieha cez
štruktúrované MQTT témy. Všetky témy zdieľajú spoločný prefix \texttt{plc}, za
ktorým nasleduje identifikátor zariadenia vo formáte \texttt{line\_\{id\}},
kde \texttt{id} zodpovedá ArUco identifikátoru zariadenia. Táto
štruktúra umožňuje systému jednoznačne smerovať správy na konkrétne zariadenia
...
identifikátormi a~históriu vykonaných príkazov. AI asistent bude nasadený ako
...
(\acrshort{cpu} 1215C DC/DC/DC). Linka má dva fotoelektrické snímače na
...
V~nasledujúcich podkapitolách popisujeme implementáciu autentifikácie a~RBAC,
...
WebSocket pre aktualizácie stavu zariadenia.
...
\begin{table}[H]
      ...
      \resizebox{\textwidth}{!}{%
            \begin{tabular}{|l|l|l|l|}
                  ...
                  \hline
                  \multicolumn{4}{|l|}{\textit{Výrobné zariadenia — /plc}}                                                                            \\
                  \hline
                  GET             & /plc/devices                               & Zoznam zariadení                  & všetci                   \\
                  POST            & /plc/devices                               & Vytvorenie zariadenia             & admin, údržbár           \\
                  GET             & /plc/devices/\{aruco\_id\}                 & Detail zariadenia                 & všetci                   \\
                  PUT             & /plc/devices/\{aruco\_id\}                 & Úprava zariadenia                 & admin, údržbár           \\
                  DELETE          & /plc/devices/\{aruco\_id\}                 & Vymazanie zariadenia              & admin, údržbár           \\
                  POST            & /plc/devices/\{aruco\_id\}/command/\{cmd\} & Odoslanie príkazu                 & admin, údržbár, operátor \\
                  ...
                  POST            & /devices/\{aruco\_id\}/login               & Prihlásenie na zariadenie         & admin, údržbár, operátor \\
                  POST            & /devices/\{aruco\_id\}/logout              & Odhlásenie zo zariadenia          & všetci                   \\
                  \hline
                  \multicolumn{4}{|l|}{\textit{Logy — /command-logs}}                                                                         \\
                  \hline
                  GET             & /command-logs                              & Logy s filtrami                   & admin, manažér, údržbár  \\
                  GET             & /command-logs/device/\{aruco\_id\}         & Logy zariadenia                   & admin, manažér, údržbár  \\
                  \hline
                  \multicolumn{4}{|l|}{\textit{Používatelia — /users}}                                                                        \\
                  \hline
                  GET             & /users                                     & Zoznam používateľov               & admin, manažér           \\
                  POST            & /users/\{user\_id\}/approve                & Schválenie + rola                 & admin, manažér           \\
                  PUT             & /users/\{user\_id\}                        & Úprava účtu                       & admin, manažér           \\
                  POST            & /users/\{user\_id\}/reset-password         & Reset hesla                       & admin, manažér           \\
                  DELETE          & /users/\{user\_id\}                        & Vymazanie účtu                    & admin, manažér           \\
                  \hline
                  ...
                  PUT             & /kb/\{entry\_id\}                          & Úprava záznamu                    & admin, manažér, údržbár  \\
                  DELETE          & /kb/\{entry\_id\}                          & Vymazanie záznamu                 & admin, manažér, údržbár  \\
                  \hline
            \end{tabular}%
      }
\end{table}
...
Pred ovládaním zariadenia sa musí používateľ na zariadenie prihlásiť. Relácia
je väzba medzi používateľom a zariadením, ktorá zabraňuje súčasnému ovládaniu
jedného zariadenia viacerými používateľmi. Na každom zariadení môže byť aktívna
práve jedna relácia.

Prihlásenie na zariadenie prebieha volaním \texttt{POST
      /devices/\{device\_id\}/login}. Backend overí, či je zariadenie voľné. Ak áno,
vytvorí novú reláciu. Ak je zariadenie obsadené, správanie závisí od parametra
\texttt{force} a od roly aktuálneho držiteľa relácie. Zariadenie obsadené
používateľom s rolou údržbára nemôže byť prebraté žiadnym iným používateľom.
...
každý, ak sa nejedná o reláciu údržbára. Je to z dôvodu, aby sa zabránilo
situáciám, keď sa operátor zabudne odhlásiť a zariadenie zostane zablokované.
...
Príkaz prechádza niekoľkými vrstvami systému, kým sa prejaví na zariadení. 
...
Pre výrobu fyzických identifikátorov bola implementovaná vlastná Python trieda
...
\begin{itemize}
      \item \textbf{AKTUÁLNA RELÁCIA} -- meno a~rola prihláseného používateľa,
            prípadne zariadenie, na ktoré je prihlásený.
      \item \textbf{KONTEXT PRE ZARIADENIE} -- výsledky vyhľadávania z~ChromaDB
            filtrované podľa ID zariadenia.
...
5. Ak sa používateľ pýta na svoje meno, rolu alebo zariadenie, odpoveď nájdeš v sekcii AKTUÁLNA RELÁCIA.
6. Ak sa používateľ pýta na konkrétne zariadenie, prednostne použi informácie z KONTEXTU PRE ZARIADENIE.
...
záznam má názov, obsah a voliteľne priradené ID zariadenia. Toto priradenie
umožňuje vyhľadávanie obsahu filtrovaného pre konkrétne zariadenie, čo zvyšuje
presnosť odpovedí pri otázkach viazaných na konkrétne zariadenie.
...
hodnoty premennej zo zariadenia. Keď táto zmena dorazí, zobrazí sa snackbar
...
\begin{table}[h]
      ...
      \begin{tabular}{lcccc}
            ...
            \midrule
            Ovládanie zariadenia         & \checkmark        & ---              & \checkmark       & \checkmark             \\
            Zobrazenie zoznamu zariadení & ---               & \checkmark       & \checkmark       & \checkmark             \\
            Správa zariadení             & ---               & ---              & \checkmark       & \checkmark             \\
...
počet zariadení online a offline, takisto počty používateľov.
...
identifikátor umiestnený priamo na zariadení a systém ho automaticky
...
umiestni na zariadenie, čím sa prepojí záznam v systéme s reálnym zariadením v
prevádzke.
...
zariadenia. Môže sledovať stavy senzorov, aktuálny režim
prevádzky a stav v ktorom sa zariadenie nachádza.
Okrem sledovania stavu môže používateľ zariadenie aj priamo ovládať, napríklad
...
\begin{figure}[H]
      ...
      \caption{Obrazovky monitoringu a~ovládania zariadenia}
...
\begin{figure}[H]
      ...
      \caption{Konverzačné rozhranie virtuálneho asistenta PLEXO}
      \label{fig:plexo}
\end{figure}
...
zariadenie, nastaviť jeho parametre a vygenerovať pre neho ArUco identifikátor. Pri
existujúcich zariadeniach je možné upravovať, čo sa zobrazuje v ovládacom
paneli zariadenia a v akom poradí. Taktiež je možné ich zo systému odstrániť,
ak boli vyradené z prevádzky. Vďaka tejto funkcionalite systém vždy odráža
reálny stav zariadení v prevádzke.

\begin{figure}[H]
      \centering
      \includegraphics[width=0.45\textwidth]{img/appScreenshots/Devices.png}
      \caption{Obrazovka správy zariadení}
      \label{fig:devices}
\end{figure}
...
odoslal, na ktoré zariadenie smeroval, kedy bol vykonaný a aký bol jeho výsledok.
Táto evidencia slúži na audit prevádzky, vyšetrovanie incidentov a~spätnú
kontrolu zásahov v prípade neštandardného správania zariadenia.
...
uplatní aj na komunikáciu s virtuálnym asistentom PLEXO. Zároveň si môže zmeniť
...
\hline
FP1 & Načítanie unikátneho identifikátora a spárovanie so zariadením & splnené \\
...
FP7 & Mechanizmus exkluzívnej relácie na zariadení & splnené \\
FP8 & Informácia o obsadení zariadenia a riadené prevzatie relácie & splnené \\
...
ako v~kapitole~\ref{sec:assistant}, no aplikovaná na produkčné nasadenie cez
