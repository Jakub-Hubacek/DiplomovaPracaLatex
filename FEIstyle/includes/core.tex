...
\begin{figure}[h]
      ...
      \caption{QR kód so zakódovanou URL adresou fei.stuba.sk}
      \label{fig:qrcode}
\end{figure}
...
mobilnej aplikácie. Identifikácia zariadenia prebieha skenovaním ArUco markera
umiestneného priamo na zariadení. Po naskenovaní sa aplikácia pripojí
k~zariadeniu a~obsluha bude môcť odosielať príkazy a~sledovať aktuálny stav.
...
kontajnerizované a~nasadené na edge zariadení umiestenom v~sieti továrne.
...
\begin{description}
      \item[Operátor:] Základná obsluha výrobnej linky. Prihlasuje sa na konkrétnu linku
            skenovaním ArUco kódu, odosiela riadiace príkazy a~sleduje aktuálny stav
            zariadenia v~reálnom čase. Nemá prístup k~reportom, správe vedomostnej bázy ani
            správe používateľov. Môže v prípade potreby využiť AI asistenta.
...
vyžadovali vlastný spôsob rozpoznávania. Prípadne by bolo možné použitie
frameworku ako Vuforia, čo by ale značne zvýšilo závislosť implementácie a
údržby a presne tomu sme sa chceli vyhnúť.
...
siete a celý systém by bol izolovaný. Nasledujúce podkapitoly popisujú
...
správu relácií, riadenie zariadení a samostatná služba rieši AI asistenta s RAG
pipeline. Tretím komponentom je MQTT broker Mosquitto, ktorý sprostredkúva
asynchrónnu komunikáciu medzi backendom a Node-RED. Štvrtým je Node-RED, ktorý
premosťuje svet priemyselnej komunikácie so svetom IT a periodicky číta dáta z
\acrshort{plc} prostredníctvom S7 protokolu. Posledným komponentom je samotné
\acrshort{plc} Siemens S7-1200, ktoré riadi prvky výrobnej linky. Všetky tieto
komponenty okrem mobilnej aplikácie a \acrshort{plc} bežia v Docker stacku na
edge zariadení. Jediné externé volanie smeruje z AI služby na rozhranie
Anthropic API.
...
%% nasledujucich par viet mi pride ze sa opakuju co som uz pisal v \subsection{Architektúra komponentov systému}
Backend systému tvorí dvojica nezávislých FastAPI služieb bežiacich v prostredí
Docker, ako už bolo opísané v \subsection{Architektúra komponentov systému}.
Asistent je nasadený oddelene predovšetkým z dôvodu možného budúceho prechodu na
lokálne bežiaci jazykový model. V takom prípade by bolo možné presunúť tento stack
na výkonnejší dedikovaný server bez akéhokoľvek zásahu do hlavného backendu.
V nasledujúcich podkapitolách popisujeme implementáciu jednotlivých funkcionalít
backendu, začínajúc autentifikáciou a RBAC, cez API endpointy, mechanizmus
relácií, životný cyklus príkazov až po generátor ArUco
...
Prihlásenie na zariadenie prebieha volaním \texttt{POST /devices/\{id\}/login}. Backend
...
používateľ odhlásením sa. Cudziu reláciu môže ukončiť každý, ak sa nejedná o
reláciu údržbára. Je to z dôvodu, aby sa zabránilo situáciám, keď sa operátor
zabudne odhlásiť a zariadenie zostane zablokované. Každé prevzatie relácie je
zaznamenané v logu príkazov, čo znamená dohľadateľnosť zodpovednosti. Pred
prepísaním relácie systém informuje nového používateľa o tom, kto je aktuálne
prihlásený. Následne je potrebné potvrdiť úmysel prevzatia, čím sa zabezpečí,
že k prevzatiu nepríde náhodou.
...
tri najbližšie záznamy. Zo získaných kontextov, metadát relácie a histórie
konverzácie sa zostaví systémový prompt, ktorý sa odošle na Claude API.
