\section{Analýza existujúcich riešení}\label{sec:existing_solutions}
...
platformy a integrácia \acrshort{ai} asistenta schopného pracovať s~technickou
dokumentáciou.
...
\begin{itemize}
      ...
      \item \textbf{Identifikácia zariadení:} Platforma nepodporuje skenovanie
            ...
            manuálne.
      \item \textbf{\acrshort{ai} asistent pre obsluhu:} Engineering Copilot je nástroj pre
...
Ignition Edge teda plní úlohu premostenia medzi fyzickými
...
\begin{itemize}
      ...
      \item \textbf{\acrshort{ai} asistent:} Platforma neobsahuje vstavanú funkciu
            \acrshort{ai} asistenta ani správu znalostnej bázy. Skriptovacie
...
\begin{itemize}
      ...
      \item \textbf{\acrshort{ai} asistent:} Frontline Copilot poskytuje generatívnu
...
zabezpečený webovým rozhraním založeným na \acrshort{html}5, čo umožňuje
...
\begin{itemize}
      ...
      \item \textbf{\acrshort{ai} asistent:} Industrial AI for Operations je
...
edge nasadenie a integráciu \acrshort{ai} asistenta s~vlastnou znalostnou bázou.
...
\begin{table}[h]
      ...
      \begin{tabular}{lcccc}
            ...
            \acrshort{ai} asistent s~vlastnou \acrshort{kb}        & Nie & Nie       & Nie       & Nie \\
            Open{-}source                          & Nie & Čiastočne & Nie       & Nie \\
            \hline
      \end{tabular}
      \vspace{0.3em}
...
\acrshort{ai} asistenta schopného odpovedať na otázky obsluhy na základe
...
Priemyselné komunikačné protokoly definujú spôsob, akým si zariadenia
v~prevádzke vymieňajú dáta. Voľba vhodného protokolu
ovplyvňuje výkon, interoperabilitu aj bezpečnosť celého riešenia.
V~nasledujúcich podkapitolách sú stručne predstavené štyri najčastejšie
protokoly, s~ktorými je možné sa v~priemyselnej praxi stretnúť.
...
Dnes je de facto štandardom v~priemyselnej automatizácii s~odhadovanými
...
\section{Prehľad technológií pre \acrshort{ai} asistentov}

Donedávna sa operátor musel pri riešení nečakanej situácie spoľahnúť
na technickú dokumentáciu alebo skúsenosti kolegov. Pokrok v~oblasti
veľkých jazykových modelov a~techník ich prepájania s~doménovou znalostnou
bázou otvoril alternatívu. Tou je interaktívny asistent, ktorý dokáže rovnaký
obsah sprostredkovať formou dialógu v~prirodzenom jazyku. Aby takéto
riešenie mohlo vzniknúť, treba si vysvetliť dve technológie -- veľké
jazykové modely a~spôsob, ako im sprístupniť doménové znalosti.
...
rýchle vyhľadávanie. Jednou z~populárnych \mbox{open-source}
 vektorových databáz je
...
\subsection{\acrshort{ai} asistenti v priemyselnom prostredí}
...
Aby mohol používateľ mobilnej aplikácie pristupovať ku konkrétnej výrobnej
linke, systém potrebuje spôsob, ako linku jednoznačne identifikovať.
Manuálne vyhľadávanie zo zoznamu je v~prostredí s~väčším počtom takmer
identických zariadení pomalé a~náchylné na omyl. Preto je vhodnejšie
priradiť každej linke fyzický identifikátor umiestnený priamo na nej
a~rozpoznať ho kamerou mobilného zariadenia. V~nasledujúcich podkapitolách
sú zhrnuté tri kategórie identifikátorov, ktoré boli zvažované pri návrhu
riešenia.
...
\begin{figure}[h]
      \centering
      \includegraphics[width=0.4\textwidth]{img/qrcode.png}
...
\begin{figure}[h]
      \centering
      \includegraphics[width=0.50\textwidth]{img/AprilTagVsArUco.png}
...
Aby výsledná aplikácia spĺňala očakávania prevádzky, musia byť požiadavky
jasne pomenované ešte pred návrhom architektúry. Systém vychádza z~konceptu
priemyselného internetu vecí (\acrshort{iiot}). Mobilná aplikácia slúži
ako primárne rozhranie pre obsluhu výrobnej linky a~komunikácia s~riadiacim
systémom prebieha cez edge zariadenie umiestnené priamo v~prevádzke.
...
Súčasťou systému bude \acrshort{ai} asistent, ktorý bude odpovedať na technické
...
\begin{description}
      \item[Operátor:] Základná obsluha výrobnej linky. Prihlasuje sa na konkrétne linky
            ...
            bázy ani správe používateľov. Môže v prípade potreby využiť \acrshort{ai} asistenta.
      ...
      \item[Manažér:] Zameriava sa na sledovanie udalostí. Má prístup k~histórii prihlásení
            ...
            Takisto má prístup k~\acrshort{ai} asistentovi a môže upravovať znalostnú bázu.
...
Nasledujúce scenáre popisujú typické situácie, v~ktorých používatelia
so~systémom pracujú. Každý scenár obsahuje identifikátor, aktéra
a~stručný popis priebehu.
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
      \toprule
      \textbf{ID}    & UC-07: \acrshort{ai} asistent                                                                                                                                                       \\
      \textbf{Aktér} & Všetky roly                                                                                                                                                               \\
      \textbf{Popis} & Tlačidlom v~rohu každej obrazovky sa spustí \acrshort{ai} asistent.
...
Definujú konkrétne schopnosti a~akcie, ktoré musí systém vedieť vykonať.
...
\begin{description}
      ...
      \item [FP5:] Implementácia \acrshort{ai} asistenta využívajúceho metódu \textit{Retrieval-Augmented Generation}. 
      \item [FP6:] Plná lokalizácia používateľského rozhrania a odpovedí \acrshort{ai} asistenta v dvoch jazykoch: slovenský (SK) a anglický (EN).
      ...
      \item [FP10:] Systém musí ukladať základný log operácií obsahujúci minimálne: čas vykonania, identifikátor používateľa, identifikátor linky, typ akcie/príkazu a výsledok vykonania. Log musí byť dostupný pre roly \textit{manažér}, \textit{údržbár} a \textit{administrátor}.
\end{description}
...
\begin{description}
      ...
      \item [NFP4:] Odpovede \acrshort{ai} asistenta musia byť striktne viazané na poskytnutý kontext. Ak informácia v materiáloch chýba, model musí túto skutočnosť priznať a môže vyhľadať informáciu na internete, no musí to uviesť.
...
\begin{description}
      ...
      \item [DP3:] Systém si musí zachovať kľúčovú funkcionalitu, teda ovládanie výrobnej linky aj v prípade výpadku pripojenia k internetu.
\end{description}
...
Tabuľka~\ref{tab:tech_choices} sumarizuje hlavné voľby technológií, zvážené
alternatívy a~stručné odôvodnenie. Detailnejšie porovnanie každej voľby je
v~príslušnej podsekcii.

\begin{table}[H]
      \centering
      \caption{Súhrn zvolených technológií a~zvážených alternatív}
      \label{tab:tech_choices}
      \renewcommand{\arraystretch}{1.3}
      \small
      \begin{tabularx}{\textwidth}{|l|l|l|X|}
            \hline
            \textbf{Vrstva}              & \textbf{Alternatívy}         & \textbf{Voľba}     & \textbf{Hlavný dôvod}                                                  \\
            \hline
            Komunikácia s~\acrshort{plc} & \acrshort{opcua}             & S7 protokol        & Natívna podpora Siemens S7-1200, bez licencie, nízka záťaž PLC         \\
            \hline
            Middleware                   & Vlastný Python skript        & Node-RED           & Vizuálne programovanie, hotové uzly pre S7 a~\acrshort{mqtt}    \\
            \hline
            Transportný protokol         & \acrshort{opcua}             & \acrshort{mqtt}    & Nenáročnosť, pub/sub architektúra, QoS úrovne                          \\
            \hline
            Backend                      & Django, Node.js, Go          & FastAPI            & Asynchrónne ASGI spracovanie, Python ekosystém pre AI                  \\
            \hline
            Frontend                     & Natívny Kotlin, React Native & Flutter            & Jedna kódová základňa, rýchly vývoj, konzistentné UI                   \\
            \hline
            Identifikátor                & 3D identifikátor, Vuforia    & ArUco              & Robustná detekcia v~OpenCV, rozšírenosť v~priemysle (AGV, roboty)      \\
            \hline
      \end{tabularx}
\end{table}
...
výpočtové zdroje \acrshort{plc} s obmedzenou pamäťou, potreba licencie a
komplexnejší konfiguračný proces. S7 protokol cez knižnicu
...
rovnako podporou \acrshort{mqtt}. Toto riešenie nevyžaduje rozsiahly
kód, čo výrazne skracuje čas konfigurácie. Node-RED tiež dobre zvláda
periodické dotazovanie \acrshort{plc} a podmienené spúšťanie akcií.
...
sústrediť sa na implementáciu biznis logiky a~integráciu \acrshort{ai} asistenta.

\begin{itemize}
      \item \textbf{Výhody:} Asynchrónne spracovanie požiadaviek, automatická OpenAPI dokumentácia, Pydantic validácia, \acrshort{ai} knižnice, rýchly vývoj.
...
bolo možné použiť framework ako Vuforia, čo by ale značne zvýšilo náročnosť
implementácie a údržby a presne tomu sme sa chceli vyhnúť.

\section{Experimentálne porovnanie a výber jazykových modelov}

Pre nasadenie \acrshort{ai} asistenta v~lokálnom prostredí je potrebné
vybrať jazykový model, ktorý dokáže vrátiť kvalitné odpovede v~rozumnom
čase aj na bežnom hardvéri. V~tejto kapitole je porovnaných päť kandidátov
rôznej veľkosti dostupných cez platformu Ollama. Po popise testovaných
modelov a~metodiky testovania nasledujú namerané výsledky a~záverečné
...
\subsection{Analýza výsledkov a výber najvhodnejšieho modelu}
...
\subsubsection{Záverečné rozhodnutie o architektúre \acrshort{ai} asistenta}
...
ako OpenAI, Google, Anthropic a iní veľkí hráči v oblasti \acrshort{ai}. Zvolili sme
\acrshort{api} od spoločnosti Anthropic, pretože v poslednom čase zaznamenali
...
jazykový model, komunikácia s \acrshort{ai} asistentom by prebiehala výhradne v rámci
...
správu relácií a~riadenie výrobnej linky. Samostatná služba obsluhuje \acrshort{ai} asistenta
...
smeruje z~\acrshort{ai} služby na rozhranie Anthropic API.
...
hlavný backend aj \acrshort{ai} asistent a~všetka logika spracovania príkazov sa
...
Backend sa pri štarte prihlasuje na odber témy \texttt{plc/\#}, čím prijíma správy zo
všetkých výrobných liniek naraz. Node-RED odoberá selektívne len témy
...
identifikátormi a~históriu vykonaných príkazov. \acrshort{ai} asistent je nasadený ako
...
Endpointy pre \acrshort{ai} asistenta sú dostupné na samostatnej službe počúvajúcej na
...
\begin{table}[H]
      ...
      \resizebox{\textwidth}{!}{%
            \begin{tabular}{|l|l|l|l|}
                  ...
                  \hline
                  \multicolumn{4}{|l|}{\textit{\acrshort{ai} asistent— port 8100}}                                                                      \\
                  \hline
                  POST            & /chat                                      & Správa \acrshort{ai} asistentovi             & všetci                   \\
...
každý, ak nejde o reláciu údržbára. Cieľom je zabrániť
...
Príkaz prechádza niekoľkými vrstvami systému, kým sa prejaví na výrobnej linke. 
...
Na výrobu fyzických identifikátorov bola implementovaná vlastná Python
trieda \texttt{ArucoSTL\-Generator\-Voxel\-Optimized}, ktorá generuje STL
súbor určený na
...
\section{Implementácia \acrshort{ai} asistenta}
...
anglickej verzii podľa jazyka relácie. Pozostáva z dvoch častí. 

Prvou je sada
pravidiel, ktorá určuje správanie asistenta, jazyk odpovede, formát, zákaz
vymýšľania faktov a spôsob práce s kontextom. 
\newpage
Druhú časť tvoria dynamicky
...
\begin{lstlisting}[style=code-listing, language=Python, caption={Základ systémového promptu \acrshort{ai} asistenta PLEXO}, label={lst:system_prompt}]
...
\texttt{\{command, value\}} a~spustí sa 5 sekundový časovač. Server najskôr
...
s~potvrdenou hodnotou a~nameranou latenciou. Ak 5 sekundový časovač vyprší
...
\begin{table}[h]
      ...
      \begin{tabular}{lcccc}
            ...
            \acrshort{ai} asistent                 & \checkmark        & \checkmark       & \checkmark       & \checkmark             \\
            \bottomrule
      \end{tabular}
\end{table}
...
s~ktorými sa používateľ pri práci s aplikáciou stretáva.
...
\end{figure}
\newpage
\subsection{\acrshort{ai} asistent PLEXO}
\textbf{Dostupné pre:} operátor, údržbár, manažér, administrátor

\acrshort{ai} asistent PLEXO je inteligentný pomocník dostupný priamo v aplikácii.
...
\begin{figure}[H]
      ...
      \caption{Konverzačné rozhranie \acrshort{ai} asistenta PLEXO}
      \label{fig:plexo}
\end{figure}
\newpage
...
Znalostnú bázu tvorí súbor informácií, z ktorých čerpá \acrshort{ai} asistent PLEXO
...
uplatní aj na komunikáciu s \acrshort{ai} asistentom PLEXO. Zároveň si môže zmeniť
...
\begin{figure}[H]
      \centering
      \includegraphics[width=0.7\textwidth]{img/PLC_Line.png}
      \caption{Model dopravníka Fischertechnik a~PLC Siemens S7-1200}
      \label{fig:plc_line}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Overenie funkcionálnych požiadaviek}
\label{tab:test-fp}
...
FP5 & \acrshort{ai} asistent s~RAG  & splnené \\
FP6 & Lokalizácia rozhrania a \acrshort{ai} asistenta v SK a EN & splnené \\
...
\begin{table}[H]
\centering
\caption{Overenie nefunkcionálnych požiadaviek}
\label{tab:test-nfp}
\begin{tabular}{|l|p{12cm}|l|}
\hline
\textbf{ID} & \textbf{Stručný popis} & \textbf{Stav} \\
...
Všetky funkcionálne aj nefunkcionálne požiadavky boli vyhodnotené ako splnené. Doménová požiadavka DP3 bola overená odpojením edge zariadenia od WAN portu, pričom mobilná aplikácia naďalej úspešne odosielala príkazy aj prijímala stav linky. Jediný komponent, ktorý je v takomto režime nedostupný, je \acrshort{ai} asistent, čo zodpovedá architektonickému rozhodnutiu opísanému v kapitole \ref{sec:assistant}.
...
Vďaka dvojfarebnému tlačenému vyhotoveniu identifikátora je kontrast dostatočný aj v priestoroch so slabším osvetlením.

\subsection{Kvalita odpovedí \acrshort{ai} asistenta}
...
Tabuľka~\ref{tab:final_comparison} sumarizuje, ako sa výsledná aplikácia
Scan2Control radí medzi existujúce riešenia opísané v~kapitole~\ref{sec:existing_solutions}.
Pôvodne identifikované medzery, najmä chýbajúca natívna mobilná aplikácia,
identifikácia zariadení a~integrácia \acrshort{ai} asistenta s~vlastnou
znalostnou bázou sú v~navrhnutom riešení pokryté.

\newcommand{\ano}{\textcolor{green!55!black}{\textbf{Áno}}}
\newcommand{\nie}{\textcolor{red!75!black}{\textbf{Nie}}}
\newcommand{\ciast}{\textcolor{black}{\textbf{Čiastočne}}}

\begin{table}[H]
      \centering
      \caption{Porovnanie Scan2Control s~existujúcimi riešeniami}
      ...
      \begin{tabular}{lccccc}
            ...
            Edge nasadenie                              & \ano & \ano   & \ciast  & \ano & \ano      \\
            \acrshort{ai} asistent s~vlastnou \acrshort{kb}        & \nie & \nie   & \nie    & \nie & \ano      \\
            Open{-}source                          & \nie & \ciast & \nie    & \nie & \ano      \\
            \hline
      \end{tabular}
\end{table}
