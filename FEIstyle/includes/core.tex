...
Moderné priemyselné aplikácie nestoja na jednej technológii. Sú výsledkom starostlivo zvolenej skladačky nástrojov, kde každá vrstva plní svoju úlohu: mobilné rozhranie reaguje na pokyny operátora a backendová vrstva spracúva množstvo súbežných udalostí z~výrobnej linky.

Kľúčovou výzvou je pritom premostenie dvoch oddelených svetov. \textbf{OT} (Operational Technology) označuje svet priemyselného riadenia -- PLC automaty, senzory, pohony a výrobné linky, ktoré fyzicky ovládajú stroje v~reálnom čase. \textbf{IT} (Information Technology) je naopak svet softvéru, sietí a aplikácií určených na spracovanie a prenos informácií. Tieto dva svety dlho fungovali oddelene, každý s~vlastnými protokolmi a prioritami. Priemyselné aplikácie v~kontexte Industry~4.0 ich spájajú práve prostredníctvom komunikačnej vrstvy -- napríklad pomocou protokolov MQTT alebo OPC~UA.

Nasledujúci prehľad mapuje najpouž\'{i}vanejšie technológie v~každej z~týchto vrstiev a ich typické uplatnenie v~priemyselnom prostredí.

\subsection{Frontendové technológie}

Pri výbere frontendovej technológie pre priemyselné aplikácie je kľúčové zvážiť cieľovú platformu (operátorský panel, tablet v teréne, manažérsky dashboard) a požiadavky na odozvu rozhrania pri spracovaní real-time dát.

\subsubsection{Natívny vývoj (Kotlin, Swift)}
Natívny vývoj predstavuje prístup, kde je zdrojový kód písaný v jazyku a nástrojoch priamo podporovaných výrobcom operačného systému (Google pre Android, Apple pre iOS). Tento prístup využíva kompiláciu do strojového kódu danej architektúry (napr. ARM), čo umožňuje priamu interakciu s jadrom systému bez sprostredkovateľských vrstiev.
...
React Native je multiplatformový framework od spoločnosti Meta, ktorý využíva paradigmu deklaratívneho programovania v jazyku JavaScript/TypeScript. Na rozdiel od hybridných webových aplikácií nevykresľuje rozhranie v prehliadači, ale cez špecializovanú vrstvu (Bridge alebo novšie JSI - JavaScript Interface) volá priamo natívne UI komponenty systému.
...
\begin{itemize}
    ...
    \item \textbf{Nevýhody:}
    \begin{itemize}
        \item \textbf{Abstrakčná réžia:} Komplexné animácie alebo masívny tok dát z PLC môžu naraziť na limity výkonu JavaScriptového vlákna.
...
Flutter je moderný UI toolkit od spoločnosti Google určený na vývoj natívne kompilovaných aplikácií z jednotnej kódovej základne v jazyku Dart. Na rozdiel od iných frameworkov nevyužíva systémové komponenty, ale disponuje vlastným grafickým jadrom (Skia/Impeller), ktoré vykresľuje rozhranie pixel po pixeli priamo na GPU.
...
Moderné webové frameworky umožňujú vytvárať komplexné klientske aplikácie, kde sa celá logika zobrazenia vykonáva v prehliadači používateľa. Využívajú techniku \textit{Virtual DOM} (React/Vue) alebo \textit{Incremental DOM} (Angular) na optimalizáciu prekresľovania len tých častí obrazovky, ktoré sa skutočne zmenili.
...
Backend v priemyselnom kontexte funguje ako inteligentný middleware, ktorý sprostredkováva obojsmerný tok dát medzi deterministickým svetom PLC a dynamickým svetom používateľských aplikácií.

\subsubsection{Node.js a NestJS}
Node.js je behové prostredie postavené na engine V8, ktoré využíva neblokujúci, udalosťami riadený I/O model. NestJS nad ním buduje architektúru inšpirovanú Angularom (Dependency Injection, modulárnosť), čo prináša do JavaScriptového sveta poriadok potrebný pre enterprise systémy.
...
Python je vďaka svojej syntaxi a obrovskej vedeckej základni dominantným jazykom pre prácu s dátami. FastAPI využíva moderné štandardy ako typové anotácie a ASGI (Asynchronous Server Gateway Interface) na dosiahnutie vysokého výkonu pri zachovaní jednoduchosti.
...
Go je kompilovaný, staticky typovaný jazyk navrhnutý v Google s dôrazom na jednoduchosť, bezpečnosť a masívny paralelizmus. Jeho kľúčovou vlastnosťou sú \textit{goroutines} – extrémne ľahké vlákna spravované runtime-om jazyka.
...
V ére Industry 4.0 sa spracovanie dát presúva z cloudu na \textbf{Edge} – k zdroju dát. Tento prístup znižuje latenciu, zvyšuje bezpečnosť (dáta neopúšťajú lokálnu sieť) a šetrí prenosové pásmo. 



\subsubsection{Docker a Docker Compose}
Docker využíva technológie jadra Linuxu (namespaces, cgroups) na izoláciu procesov do tzv. kontajnerov. Na rozdiel od virtuálnych strojov kontajnery nezahrňujú celý operačný systém, ale zdieľajú jadro hostiteľa, čo ich robí extrémne ľahkými.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Docker Compose:} Umožňuje definovať celý aplikačný stack (napr. InfluxDB pre dáta, Mosquitto pre správy a FastAPI pre logiku) v jednom konfiguračnom súbore, čím sa orchestrácia Edge uzla stáva triviálnou.
    \end{itemize}
...
OPC UA je platformovo nezávislý štandard, ktorý nahrádza staršie riešenia založené na Microsoft COM/DCOM. Definuje nielen transport, ale aj informačný model (uzly, objekty, premenné).
...
Ide o natívnu komunikáciu pre PLC Siemens. Využíva ISO on TCP (RFC 1006) a umožňuje nízkoúrovňový prístup k pamäťovým oblastiam automatu.
...
MQTT je ľahký binárny protokol typu publish-subscribe, optimalizovaný pre IoT. Správy sa neposielajú priamo, ale cez sprostredkovateľa (broker).
...
Najrozšírenejší "legacy" protokol založený na architektúre Client/Server. Pracuje s jednoduchým modelom registrov (Holding, Input) a cievok (Coils).
...
\subsection{QR kódy}
QR kódy sú dvojrozmerné maticové identifikátory optimalizované pre rýchle čítanie a vysokú hustotu dát. V kontexte priemyselnej automatizácie sa využívajú najmä na statickú identifikáciu objektov, kde sa vyžaduje uloženie komplexnejších informácií napr. URL adresy, sériové čísla alebo konfiguračné parametre. Ich výhodou je silná korekcia chýba, ktorá umožňuje prečítať kód aj pri čiastočnom poškodení alebo znečistení povrchu.
...
Tieto identifikátory sa vyznačujú predovšetkým vysokou mierou interoperability s modernými robotickými riešeniami. Vďaka existujúcim knižniciam napr. OpenCV, ROS - Robot Operating System je ich integrácia štandardom pri vývoji:
\begin{itemize}
...
Práca predstavuje moderný prístup k priemyselnému internetu vecí (IIoT),
kde mobilná aplikácia slúži ako primárne rozhranie pre obsluhu výrobnej linky,
prepojené s riadiacim systémom prostredníctvom edge computing zariadenia umiestneného priamo v prevádzke.

\subsection{Prehľad systému}

V moderných priemyselných prevádzkach narastá tlak na flexibilitu a mobilitu obsluhy výrobných zariadení.
Tradičné riešenia sú spravidla viazané na pevne umiestnené operátorské panely alebo stolové počítače,
čo obmedzuje pohyblivosť obsluhy a~zvyšuje reakčný čas pri riešení prevádzkových udalostí.
Mobilné zariadenia ponúkajú možnosť prístupu k výrobným systémom odkiaľkoľvek v rámci areálu závodu.
Ich použitie v~kombinácii s~jednoznačnou identifikáciou zariadení otvára cestu
k~bezpečnému a~intuitívnemu ovládaniu výrobnej linky.

Kľúčovou výzvou pri takomto prístupe je spoľahlivé a~jednoznačné spárovanie mobilného zariadenia
s~konkrétnou fyzickou časťou výrobnej linky.
Skenovaním vizuálneho identifikátora umiestneného priamo pri zariadení operátor okamžite nadviaže spojenie a systém presne vie, kto a~ktoré zariadenie ovláda.
Toto spojenie je základom pre bezpečné riadenie, auditovanie operácií
a~správu prístupu viacerých používateľov k rovnakému zariadeniu.
...
Celá komunikačná vrstva od PLC až po mobilnú aplikáciu prebieha cez štandardné priemyselné
a~webové protokoly. Backendová infraštruktúra je plne kontajnerizovaná a
to umožňuje rýchle nasadenie na ďalšie uzly v~rámci továrne.
Ovládanie linky je obmedzené výhradne na lokálnu sieť továrne a~cloudová vrstva slúži len na vzdialený reporting a~zálohovanie agregovaných dát.

Neoddeliteľnou súčasťou systému je inteligentný technický asistent.
Asistent odpovedá na technické otázky obsluhy výhradne na základe dokumentácie a~manuálov
nahratých pre konkrétne zariadenie, čím znižuje závislosť od papierových príručiek
a~skracuje čas potrebný na riešenie prevádzkových problémov.

\subsection{Popis používateľských rolí}

Systém implementuje riadenie prístupu založené na rolách
(\textit{Role-Based Access Control}, RBAC).
Každý nový používateľ sa musí zaregistrovať a~pred prvým prihlásením čaká na schválenie
administrátorom, ktorý mu zároveň priradí príslušnú rolu. Sú definované štyri roly:

\begin{description}
    \item[Operátor:] Základná obsluha výrobnej linky. Prihlasuje sa na konkrétnu linku skenovaním
    ArUco kódu, odosiela riadiace príkazy, upravuje prevádzkové parametre
    a~sleduje aktuálny stav zariadenia v~reálnom čase.
    Nemá prístup k~reportom, správe vedomostnej bázy ani správe používateľov. 
    Môže v prípade potreby využiť AI asistenta.

    \item[Manažér:] Zameriava sa na sledovanie výkonnostných ukazovateľov.
    Má prístup k~histórii prihlásení jednotlivých operátorov,
    dennému reportu o~vyťaženosti a~agregovaným štatistikám pre všetky zariadenia.
    Linku priamo neovláda.
    ...
    \item[Administrátor:] Spravuje celý systém, schvaľuje nové registrácie, priraďuje roly,
    edituje a~odstraňuje používateľské kontá (s~výnimkou hlavného admina).
    Má prístup ku všetkým logom a~taktiež spravuje vedomostnú bázu.
\end{description}
...
aby pokrývali tri veľkostné kategórie: modely do 10 miliárd parametrov,
modely v rozsahu 10 až 15 miliárd parametrov a model s 24 miliardami parametrov.
Cieľom tohto rozloženia bolo analyzovať vzťah medzi veľkosťou modelu,
kvalitou odpovedí a časom odozvy.
...
v~jazyku Python s~využitím knižníc LangChain, ChromaDB a Ollama.
Znalostná báza asistenta PLEXO vo formáte \texttt{.txt} bola rozdelená
na dokumentové sekcie podľa markerov \texttt{--- DOKUMENT: ---}
a uložená do vektorovej databázy ChromaDB s~využitím embedding modelu
\texttt{nomic-embed-text}. Pre každú testovú otázku boli z~vektorovej
databázy vytiahnuté tri najrelevantnejšie dokumenty, ktoré boli spolu
s~otázkou odovzdané modelu ako kontext.

Každý model bol otestovaný na sade 14 testovacích otázok rozdelených
do troch kategórií:
...
Na rozdiel od automatických metrík ako ROUGE, ktoré vykazujú nízku
spoľahlivosť pri skloňovaných jazykoch ako slovenčina, bolo hodnotenie
kvality realizované manuálne. Každá odpoveď bola ohodnotená
...
relevancia (30\,\%), úplnosť (25\,\%), zrozumiteľnosť (20\,\%)
a jazyková správnosť (25\,\%).

Okrem kvalitatívnych metrík bol pre každú odpoveď zaznamenaný
čas odozvy v~milisekundách. Testovanie prebiehalo na notebookovom
hardvéri (CPU AMD Ryzen 5 5600H, RAM 32\,GB, GPU NVIDIA RTX~3060 6\,GB VRAM).
Keďže väčšina testovaných modelov presahuje kapacitu VRAM,
modely bežali čiastočne alebo úplne na systémovej RAM,
čo predstavuje hardvérovú limitáciu meraní.
V~produkčnom nasadení na dedikovanom serveri s~dostatočnou VRAM
by časy odozvy boli výrazne nižšie.
...
Z~výsledkov testovania vyplýva, že model \textbf{gemma2:9b} dosiahol
najvyššie celkové skóre 4,92/5,0 spomedzi všetkých testovaných modelov.
Model vykazoval konzistentne vysokú kvalitu odpovedí naprieč všetkými
typmi otázok aj oboma testovacími jazykmi, pričom jeho priemerný čas
odozvy 11\,165\,ms je takmer trojnásobne nižší v~porovnaní
s~modelom mistral-small:24b.

Pozoruhodným zistením je skutočnosť, že modely phi4:14b
a mistral-small:24b opakovane odmietali odpovedať na otázky,
ktorých odpovede boli v~znalostnej báze jednoznačne obsiahnuté.
Tento jav naznačuje, že retrieval pipeline nepodal týmto modelom
dostatočne relevantný kontext, čo mohlo byť spôsobené odlišnou
citlivosťou modelov na formát vstupného kontextu alebo
prísnejším nastavením ich systémového promptu.

Všetky testované modely dosiahli 100\,\% úspešnosť pri odmietaní
otázok mimo rozsahu znalostnej bázy, čo potvrdzuje správne
fungovanie systémového promptu z~hľadiska bezpečnosti systému.

Na základe výsledkov testovania bol pre integráciu do mobilnej
aplikácie vybraný model \textbf{gemma2:9b}. Tento model poskytuje
najlepší pomer medzi kvalitou odpovedí a časom odozvy,
je konzistentný v~slovenskom aj anglickom jazyku
a jeho veľkosť 9 miliárd parametrov je vhodná pre nasadenie
na serverovom hardvéri s~bežnou VRAM kapacitou.
